    ...
    vim.api.nvim_create_autocmd(
        { "BufEnter", "BufWinEnter" },
        {
            ...
            callback = function()
                ...
                vim.keymap.set(
                    'n', '<localleader>t', markdown.toggle_checkbox, { buffer = true, desc = "Toggle checkbox" }
                )
                vim.keymap.set('n', '<localleader>i', '<cmd>:ZettelInsertLink<cr>', { desc = "Insert link to note under the cursor" })
            end
        }
    )
end
